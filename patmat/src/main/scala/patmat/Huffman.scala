...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    @tailrec
    def helper1(xs: List[Char], acc: List[(Char, Bit)]): scala.List[(Char, Int)] = chars match {
      case Nil => acc
      case x :: xs => helper1(xs, helper2(x, Nil, acc))
    }

    chars match {
      case Nil => Nil
      case x :: xs => helper1(xs, List((x, 1)))
    }
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case List(_) => trees
    case f :: s :: tail => (List(makeCodeTree(f, s)) ::: tail).sortBy(weight)
  }
  ...
  def until(done: List[CodeTree] => Boolean, merge: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] =
    if (done(trees)) List(trees.head) else until(done, merge)(merge(trees))
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def helper(tree: CodeTree, curr: Char, acc: List[Bit]): List[Bit] = tree match {
      case Leaf(_,_)=>acc
      case Fork(l,r,_,_) => if (chars(l).contains(curr))
        helper(l, curr, acc ::: List(0))
      else helper(r, curr, acc ::: List(1))
    }
    text match {
      case x :: xs => helper(tree, x, Nil) ::: encode(tree)(xs)
      case Nil => Nil
    }
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def helper1(table: CodeTable)(text: List[Char]): List[Bit] = {
      ...
      text match {
        case Nil => Nil
        case x :: xs => helper2(table, x) ::: helper1(table)(xs)
      }
    }
