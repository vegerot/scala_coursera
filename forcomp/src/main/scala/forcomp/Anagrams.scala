...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w.toLowerCase groupBy(c=>c)).toList
    .map{ case (c, s)=>(c, s.size) }
    .sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.toMap
      x.map{case (char, cnt)=>(char, cnt - yMap.getOrElse(char, 0) )}
      .filter{case (_, cnt)=> cnt >0}.sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def helper(occurrences: Occurrences): List[Sentence] = {
      if (occurrences == Nil) List(Nil)
        ...
        valids <- dictionaryByOccurrences.getOrElse(subset, Nil)
